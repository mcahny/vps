...
def upsnet_test():
    ...
    while i_iter < len(test_loader):
        ...
        for gpu_id in gpus:
            try:
                ...
                for k, v in data.items():
                    ...
                    data[k] = v.to(gpu_id, non_blocking=True) if torch.is_tensor(v) else v
        ...
        with torch.no_grad():
            ...
            'scores' : [n]
            'boxes': [n,4]
            'masks': [n,9,28,28] things
            'ssegs': [1,1024,2048]
            'panos': [1,1024,2048]
            'cls_inds': [n]
            'pano_cls_inds': [k:num_things] things
            '''
        post_timer.tic()
    ...
    ####
    # all_boxes (list [num_inst_cls+1*500*[k,5]])
    # all_masks (list [num_inst_cls+1*500*[dict['size','counts']]])
    # all_ssegs (list [500*(1024,2048)])
    # all_panos (list [500*(1024,2048)])
    # all_pano_cls_inds (list [500*(k-cls_inds)])
    ...
    if config.test.vis_mask:
        ...
        pdb.set_trace()
        # all_ssegs (0~18; 0~10 stuff, 11~18 things), all_panos (unknown: 255) , all_pano_cls_inds ,
        if config.network.has_panoptic_head:
            logging.info('unified pano result:')
