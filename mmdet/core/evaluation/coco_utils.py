...
def segm2json(dataset, results, eval_types):
    ...
    json_semantic_segm_all_results = []
    for idx in range(len(dataset)):
        img_id = dataset.img_ids[idx]
        ...
        if 'semantic_segm' in eval_types:
            for label in range(len(semantic_segm)):
                ...
                semantic_segm_cls['counts'] = semantic_segm_cls['counts'].decode()
    ...
    if 'semantic_segm' in eval_types:
        return bbox_json_results, segm_json_results, json_semantic_segm_results, json_semantic_segm_all_results
...
def results2json(dataset, results, out_file, eval_types):
    ...
    if isinstance(results[0], list):
        ...
        result_files['semantic_segm'] = '{}.{}.json'.format(out_file, 'semantic_segm')
