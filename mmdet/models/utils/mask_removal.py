...
class MaskFcnRemoval(nn.Module):

    def __init__(self, num_seg_classes, fraction_threshold=0.3, class_mapping=None):
        super(MaskFcnRemoval, self).__init__()
        self.fraction_threshold = fraction_threshold
    ...
    def forward(self, mask_rois, cls_prob, mask_prob, cls_idx, im_shape):
        ...
        """
        mask_logit_gpu = mask_prob
        mask_energy = mask_rois.new_zeros(1, self.num_seg_classes, im_shape[0], im_shape[1])
        frame_id = 0
        ...
        for i in range(sorted_inds.shape[0]):
            ...
            keep_inds.append(sorted_inds[i])
            mask_image[cls_idx[i]][y_0:y_1, x_0:x_1] += crop_mask
            mask_energy[0, self.class_mapping[cls_idx[i]+1], y_0: y_1, x_0: x_1] += logit_tensor[(y_0 - ref_box[1]):(y_1 - ref_box[1]), (x_0 - ref_box[0]):(x_1 - ref_box[0])]
            frame_id += 1
